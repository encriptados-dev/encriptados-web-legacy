...
const CardSimEsim: FC<CardSimEsimProps> = ({
  ...
  return (
    <div className={`${background} w-full sm:rounded-3xl rounded-3xl`}>
      <div className="p-6 sm:p-9 flex flex-nowrap sm:flex-nowrap gap-6">
        {/* Texto */}
        <div className="w-[65%] sm:w-[55%]">
          <h2
            className={`text-lg sm:text-xl mb-2 font-semibold ${titleColor} leading-5`}
          >
            {title}
          </h2>
          {description && (
          ...
          {showMoreInfo && (
            <Link href={moreInfoUrl} passHref>
              <button
                ...
                className="text-gray-300 text-sm hover:text-white transition mt-6 sm:mt-14 block lg:text-left text-center w-full"
              >
                {moreInfoText}
              </button>
            </Link>
        ...
        {/* Imagen */}
        <div className="w-[30%] sm:w-[45%] self-center">
          <Image
            src={imageSrc}
            alt={altText}
            width={200}
